...
<body>
  <div class="header">
    <img src="${logoSrc}" alt="Logo" style="height:64px;width:auto;object-fit:contain;margin-bottom:8px;" onerror="this.style.display='none'" />
...
export function buildIdCardPrint(student, institutionName) {
  ...
  // Prefer admission number; else generate FT-XXXXXXXX from id
  let idNo = student.admissionNo || student.admissionNumber || student.studentId || '';
  if (!idNo && student.id) {
  ...
  const root = (document.body && document.body.dataset && document.body.dataset.root) || '';
  let logoSrc = '';
  try { logoSrc = new URL((root || './') + 'icons/logo-future-tech.png', window.location.href).href; } catch(_) {}
  return `
    <div style="display:flex;justify-content:center;padding:1rem;">
      <div style="width:360px;border:3px solid #C9A227;border-radius:14px;overflow:hidden;font-family:Inter,Arial,sans-serif;box-shadow:0 4px 20px rgba(0,0,0,0.12);">
        <div style="background:#0A1628;color:#fff;padding:12px 14px;text-align:center;border-bottom:3px solid #C9A227;">
          <img src="${logoSrc}" alt="" style="height:48px;width:auto;object-fit:contain;background:#fff;border-radius:8px;padding:2px;margin-bottom:6px;" onerror="this.style.display='none'" />
          ...
          <div style="font-size:10px;color:#E8D48B;margin-top:2px;">Qamber · Quest for Excellence</div>
        </div>
        <div style="padding:16px 14px;background:#fff;">
          <div style="display:flex;gap:12px;align-items:flex-start;">
            ...
            <div style="flex:1;font-size:12px;line-height:1.55;">
              ...
              ${roll ? `<div><strong>Roll No:</strong> ${roll}</div>` : ''}
              <div><strong>Class:</strong> ${cls}${section ? ' — ' + section : ''}</div>
